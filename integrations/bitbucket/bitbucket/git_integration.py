...
from bitbucket.v2.objects import Project
from loguru import logger
from pydantic import Field, BaseModel
from bitbucket_integration.core.async_fetcher import AsyncFetcher
from bitbucket_integration.core.entities import (
    FILE_PROPERTY_PREFIX,
    SEARCH_PROPERTY_PREFIX,
)
from bitbucket_integration.bitbucket_service import PROJECTS_CACHE_KEY
from bitbucket_integration.utils import get_cached_all_services
...
class BitbucketMemberSelector(Selector):
...
class BitbucketObjectWithMembersResourceConfig(ResourceConfig):
    kind: Literal["project-with-members", "group-with-members"]
    selector: BitbucketMemberSelector
...
class BitbucketProjectSelector(Selector):
...
class BitbucketFilesSelector(Selector):
    files: FilesSelector


class BitbucketFilesResourceConfig(ResourceConfig):
    selector: BitbucketFilesSelector
    kind: Literal["file"]


class BitbucketProjectResourceConfig(ResourceConfig):
    selector: BitbucketProjectSelector
    kind: Literal["project"]


class BitbucketPortAppConfig(PortAppConfig):
    ...
    resources: list[BitbucketObjectWithMembersResourceConfig | BitbucketFilesResourceConfig | BitbucketProjectResourceConfig | BitbucketResourceConfig] = Field(default_factory=list)  # type: ignore
...
def _get_project_from_cache(project_id: int) -> Project | None:
    ...
    for token_projects in event.attributes.get(PROJECTS_CACHE_KEY, {}).values():
        if project := token_projects.get(project_id):
            return project
    logger.info(f"Project {project_id} not found in cache, fetching from Bitbucket")
